...
public class Biblioteca {
	...
	public static void main(String[] args) {
		...
		case 2:
			...
			break;
			//En caso de que documento se trate como nulo, lanza Excepción
			}catch (Exception e) {
		...
		case 5:
			 ...
			 * Si el documento no está prestado, se mostrará que no existe ningún préstamo.
			 */
			try {
			if(documento != null && documento.isPrestado()) {
				System.out.println("Mostrando Informe de Préstamos: +\n---------------------------");
				if(documento instanceof Libros) {
					Libros libro = (Libros) documento;
					System.out.println(libro.getTitulo() + "\t(" + libro.getAñoPublicacion()+ ")\t" + libro.getCodigo() + "\tCód:" + libro.getCodigo());
				}
				if(documento instanceof Revistas) {
					Revistas revista = (Revistas) documento;
					System.out.println(revista.getTitulo() + "\tCód:" + revista.getCodigo());
				}
			}else if(!documento.isPrestado()) {
				System.out.println("Actualmente no existe ningún préstamo.");
			}
			break;
			}catch(Exception e) {
				System.out.println("No hay ningún documento seleccionado.");
