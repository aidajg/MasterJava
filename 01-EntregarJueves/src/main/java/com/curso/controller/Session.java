...
public class Session extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//Validacion para el nombre (no permitir digitos)
		String digitos = ".*[0-9].*";
		...
		try {
		    ...
		    if(pvp == null || stock == null || nombre == null || categoria == null) {
		    	...
		    	if(pvp <= 0 || stock <= 0) {
					...
					RequestDispatcher rd = request.getRequestDispatcher("/agregar.html");
					rd.include(request, response);
		    	}else if(nombre.isBlank() || nombre.isEmpty() || nombre.matches(digitos)){
					out.println("<p style=\"color:red;\">Nombre no puede contener espacios ni vacios. Tampoco números.</p>");
					RequestDispatcher rd = request.getRequestDispatcher("/agregar.html");
					rd.include(request, response);
		    	}else if(pvp > 0 && stock > 0) {
